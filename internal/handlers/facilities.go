...
func RegisterFacilityRoutes(r *gin.Engine, db *repo.DB, jwtSecret string) {
    ...
    r.POST("/facilities", authMW, func(c *gin.Context) {
        ...
        if body.Name == "" || body.Type == "" { c.JSON(http.StatusBadRequest, gin.H{"error": "name/type required"}); return }
        // 简化：直接插入
	...
	r.POST("/facilities/:id/units", authMW, func(c *gin.Context) {
		...
		if err := c.BindJSON(&body); err != nil || body.Label == "" { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid label"}); return }
	...
	r.PATCH("/units/:id", authMW, func(c *gin.Context) {
		...
		if err := c.BindJSON(&body); err != nil || body.IsActive == nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid body"}); return }
